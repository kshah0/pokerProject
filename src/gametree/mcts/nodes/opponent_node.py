...
class OpponentNode(InnerNode):
    ...
    def select_child(self) -> INode:
        return self.config.opponent_node_selection_strat.select(self)

    # @Override
    def create_back_prop_strategy(self) -> BackPropagationStrategy:
        return self.config.backpropagation_strat_factory.create_for_opponent_node(self)